...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    tweet_parts = tweet_words(tweet)
    t_sent = 0
    t_total = 0
    for x in tweet_parts:
        sentimento = make_sentiment(get_word_sentiment(x))
...
def find_centroid(polygon):
    ...
    resultado_x = 0
    resultado_y = 0
    for t in range(0, len(polygon)-1):
        area += (polygon[t][0] * polygon[t+1][1]) - (polygon[t+1][0] * polygon[t][1])
    if area == 0:
        return (polygon[0][0], polygon[0][1], area)
    area = area/2
    for t in range(0, len(polygon)-1):
        resultado_x += (polygon[t][0] + polygon[t+1][0]) * ((polygon[t][0] * polygon[t+1][1]) - (polygon[t+1][0] * polygon[t][1]))
        resultado_y += (polygon[t][1] + polygon[t+1][1]) * ((polygon[t][0] * polygon[t+1][1]) - (polygon[t+1][0] * polygon[t][1]))
    resultado_x /= (area * 6)
    resultado_y /= (area * 6)
    return  (resultado_x, resultado_y, abs(area))
...
def find_center(polygons):
    ...
    a = 0    
    for l in polygons:
        poligonos = find_centroid(l)
        x += poligonos[0] * poligonos[2]
        y += poligonos[1] * poligonos[2]
        a += poligonos[2]
    x /= a
    y /= a
    return (x, y) 
...
def most_talkative_state(term):
    ...
    "*** YOUR CODE HERE ***"
    lista_tweets = group_tweets_by_state(tweets)
    estado = 0
    contador = 0
    for x in lista_tweets:
        if len(lista_tweets[x]) > contador:
            contador = len(lista_tweets[x])
            estado = x
    return estado
